...
int animationSpeed = 10;     // Timer interval in milliseconds
double radius = 0.65; // Radius of the circle
// Mouse tracking
int lastMouseX = 0;
int lastMouseY = 0;
...
void updateClockHands()
{
    ...
    // Calculate angles
    secondAngle = 360.0f - ((seconds + milliseconds / 1000.0f) * 6.0f); // 6 degrees per second
    if (secondAngle <= 0)
    {
        secondAngle += 360.0f;
    }

    minuteAngle = 360.0f - (minutes * 6.0f) - (seconds * 0.1f); // 6 degrees per minute + 0.1 degrees per second
    if (minuteAngle <= 0)
    {
        minuteAngle += 360.0f;
    }

    hourAngle = 360.0f - (hours * 30.0f) - (minutes * 0.5f); // 30 degrees per hour + 0.5 degrees per minute
    if (hourAngle <= 0)
    {
        hourAngle += 360.0f;
    }
...
/**
 * Draws a circle centered at the origin in the XY-plane.
 *
...
void drawTicks(int cnt, double radius){

    for(int i = 0;i<cnt;i++){
        double angle = 2.0 * M_PI * i / cnt;  
        double outerX = (radius-0.05) * cos(angle);
        double outerY = (radius-0.05) * sin(angle);
        double innerX = (radius-0.1) * cos(angle);
        double innerY = (radius-0.1) * sin(angle);
        if(i%5 == 0){
            innerX = (radius-0.15) * cos(angle);
            innerY = (radius-0.15) * sin(angle);
        }
        glBegin(GL_LINES);
        {
            glVertex3f(innerX,innerY,0.0); // Start of the tick
            glVertex3f(outerX,outerY,0.0); // End of the tick
        }
        glEnd();    
    }
...
void drawHand(double length, double angle, double width)
{
    glRotatef(angle, 0.0, 0.0, 1.0);
    glBegin(GL_QUADS);
    {
        glVertex3f(-width / 2, 0.0, 0.0);       // Bottom-left
        glVertex3f(width / 2, 0.0, 0.0);        // Bottom-right
        glVertex3f(width / 2, length, 0.0);     // Top-right
        glVertex3f(-width / 2, length, 0.0);    // Top-left
    }
    glEnd();
}
/**
...
void display()
{
    ...
    glPushMatrix();
    {
        ...
        glTranslatef(0.0, 0.0, 0);    // Move to center
        drawCircle(radius, 500);        // Draw a circle with radius 0.5 and 100 segments
    }
    glPopMatrix();
    ...
    {
        glColor3f(1.0f, 1.0f, 1.0f); // Set color to white
        drawTicks(60,radius); // Draw ticks on the circle
    }
    glPopMatrix();
    ...
    {
        glColor3f(1.0f, 0.0f, 0.0f);
        drawHand(0.45, secondAngle, 0.005);  
    }
    glPopMatrix();

    glPushMatrix();
    {
        double angle = secondAngle + 90.0f;
    ...
    glPushMatrix();
    {
        ...
        drawHand(0.4, minuteAngle, 0.01);
    }
    glPopMatrix();

    glPushMatrix();
    {
        double angle = minuteAngle + 90.0f;
        double x = radius * cos(angle * M_PI / 180.0);
        double y = radius * sin(angle * M_PI / 180.0);
    ...
    {
        glColor3f(1.0f, 1.0f, 1.0f);
        drawHand(0.35, hourAngle, 0.015);
    }
    glPopMatrix();

    glPushMatrix();
    {
        double angle = hourAngle + 90.0f;
        double x = radius * cos(angle * M_PI / 180.0);
        double y = radius * sin(angle * M_PI / 180.0);
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("Clock Offline");
